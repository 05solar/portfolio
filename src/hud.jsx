...
function StatBar({ label, cur, max, color, suffix }) {
    ...
    <div className="bevel">
      ...
      <div style={{padding: 12}}>
        <div style={{display:"flex", gap:10, alignItems:"center", marginBottom:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontFamily:"var(--font-pixel)", fontSize:11, color:"var(--neon-yellow)", letterSpacing:1}}>SOLAR</div>
            <div style={{fontFamily:"var(--font-mono)", fontSize:13, color:"var(--neon-cyan)", letterSpacing:1}}>FULL-STACK MAGE</div>
          </div>
        </div>
        <StatBar label="HP" cur={D.stats.HP.cur} max={D.stats.HP.max} color="var(--neon-red)"/>
        ...
        <div style={{display:"flex", justifyContent:"space-between", marginTop:8, fontFamily:"var(--font-mono)", fontSize:13, color:"var(--ink-dim)", letterSpacing:1}}>
          <span>GOLD ◈</span>
          <span style={{color:"var(--neon-yellow)"}}>{D.stats.GOLD.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
...
function MiniMap({ playerPos, signs, house, worldW, worldH }) {
  const colorMap = {
  ...
  const innerW = 232; const innerH = 152;
  ...
  return (
    <div className="bevel">
      <div className="title-bar">
        <span>MAP</span>
        <span style={{fontFamily:"var(--font-mono)", fontSize:11, color:"var(--neon-green)"}}>OVERWORLD</span>
      </div>
      <div style={{padding: 4}}>
        <div className="minimap bevel-inset">
          <div className="minimap-inner">
            ...
            {/* Player dot */}
            <div className="minimap-dot minimap-player"
              style={{ left: px, top: py }}/>
          </div>
        </div>
        <div className="coords" style={{textAlign:"center", paddingTop:6, paddingBottom:4, fontFamily:"var(--font-mono)", fontSize:13}}>
          <span style={{color:"var(--ink-dim)"}}>POS </span>
...
function NowPlaying({ insideHouse }) {
  const track = insideHouse ? "WORKSHOP THEME — SOLAR OST" : "VILLAGE THEME — SOLAR OST";
  return (
    <div className="bevel" style={{display:"inline-block"}}>
      <div className="now-playing">
        <span className="eq-bars"><span/><span/><span/><span/></span>
        <span style={{color:"var(--ink-dim)", letterSpacing: 1}}>♪ NOW PLAYING</span>
        <span>{track}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="bevel" style={{display:"inline-block"}}>
      <div style={{padding:"8px 14px", display:"flex", gap:14, alignItems:"center", fontFamily:"var(--font-mono)", fontSize:14, letterSpacing:1}}>
        ...
        <span style={{color:"var(--ink-mute)"}}>|</span>
        <span style={{color:"var(--neon-magenta)"}}>SAVE-3</span>
      </div>
    </div>
  );
}
